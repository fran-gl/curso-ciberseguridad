...
func (m commit) Insert(ctx context.Context, commit *entity.Commit) (err error) {
	...
	`
	// usar placeholders para evitar SQL Injection -> se toma cada dato como un string independiente? Sin concatenar
	// comentario clase:
	// Para prevenir un SQL Injection, es esencial utilizar consultas parametrizadas
	// en lugar de concatenar cadenas directamente en las consultas SQL.
	// Esto asegura que los datos ingresados por el usuario no se interpreten como código SQL.
	// ORM y limpiar datos ingresados por los usarios tambien ayuda
